...
contract('Main contract', (accounts) => {
    ...
    it('set_sell test', async() => {
        ...
        assert.equal(res.logs[0].args.creator, accounts[0], 'nft creator is account[0]')
        assert.equal(res.logs[0].args.setter, accounts[0], 'setter who set nft as selling list is accounts[0]')
        
        const balance_0 = await afen.balanceOf(accounts[0])
        const balance_main = await afen.balanceOf(main.address)
        console.log('balance_0: ', balance_0.toString())
        console.log('balance_main: ', balance_main.toString())
        assert.equal(balance_main.toString(), 1004, 'selling list fee is correctly 4%')
